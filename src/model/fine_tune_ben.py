...
def train_model(fc_units=64, 
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        model = BigEarthNetFM(fc_units=fc_units, dropout=dropout).to(device)
        
        # freeze base model, keep LoRA adapters trainable
        for name, param in model.pretrained_model.named_parameters():
            if "lora_" in name or "bias" in name:  # PEFT adds LoRA modules with this pattern
