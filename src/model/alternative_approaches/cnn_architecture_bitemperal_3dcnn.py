...
class BiTempCNNBranch(nn.Module):
    def __init__(self, dropout=0.4):
        super().__init__()
        # First 3D convolution: input (B, C=2, T=2, H, W)
        self.conv3d = nn.Sequential(
            nn.Conv3d(2, 16, kernel_size=(2, 3, 3), padding=(0, 1, 1)),  # → (B, 16, 1, H, W)
            nn.BatchNorm3d(16),
            nn.ReLU()
        )

        # Remaining 2D CNN layers
    ...
    def forward(self, x):
        # Input: (B, 4, H, W) → separate channels
        # Current order: [VV_post, VH_post, VV_pre, VH_pre]
        B, C, H, W = x.shape
        assert C == 4, "Expected 4 channels: VV_post, VH_post, VV_pre, VH_pre"

        # Rearranged into (B, 2, 2, H, W): C=2 (VV, VH), T=2 (pre, post)
        x = torch.stack([
            torch.stack([x[:, 2, :, :], x[:, 0, :, :]], dim=1),  # VV: pre, post
            torch.stack([x[:, 3, :, :], x[:, 1, :, :]], dim=1),  # VH: pre, post
        ], dim=1)  # → shape (B, 2, 2, H, W)

        x = self.conv3d(x)              # → (B, 16, 1, H, W)
        x = x.squeeze(2)                # → (B, 16, H, W)
        x = self.features2d(x)          # → (B, 64, 8, 8)
        return x
...
class MultiModalCNN(nn.Module):
    def __init__(self, fc_units=128, fusioned_kernel_units=128, dropout=0.4, final_dropout=0.4):
        ...
        self.sar_desc_branch = BiTempCNNBranch(dropout)
        self.sar_asc_branch = BiTempCNNBranch(dropout)
